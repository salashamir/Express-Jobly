...
class User {
  ...
  static async get(username) {
    ...
    user.jobs = jobApplications.rows.map((j) => j.job_id);

    return user;
  }

  /** Method for applying to a job with a username and the job id
   * Returns job_id
   *
   * Throws NotFoundError if job ir or username not found
   */
  ...
  static async applyToJob(username, jobID) {
    ...
    if (!user.rows[0]) {
      throw new NotFoundError(`User does not exist with username: ${username}`);
    }
